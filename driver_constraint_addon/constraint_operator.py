...
class CreateDriverConstraint(bpy.types.Operator):
    ...
    min_value : bpy.props.FloatProperty(name = "Min Value",default=0.000000, description="That value is used as 0.0 value for the shapekey.")
    max_value : bpy.props.FloatProperty(name = "Max Value",default=1.000000, description="That value is used as 1.0 value for the shapekey.")
    ...
    prop_min_value : bpy.props.FloatProperty(name = "Min Value",default=0.000000, description="That value is used as 0.0 value for the Property.")
    prop_max_value : bpy.props.FloatProperty(name = "Max Value",default=1.000000, description="That value is used as 1.0 value for the Property.")
    ...
    def create_property_driver(self,wm,context,scene,active_object):
                ...
                for curve in curves:
                    if curve != None:
                        ...
                        if self.type in ["SCALE_X","SCALE_Y","SCALE_Z"]:
                            curve.driver.expression = "max({}-1,(var-1)/({}-1))".format(round(min_value,3),round(max_value,3))
                        else:
                            curve.driver.expression = "max({},var/{})".format(round(min_value,3),round(max_value,3))
        ...
        if driver_found:
            msg = self.prop_data_path +" Driver has been added."+'''
                        ...
                        max value = {}
                        prop_max_value = {}
                        '''.format(min_value,self.prop_min_value,max_value,self.prop_max_value)
